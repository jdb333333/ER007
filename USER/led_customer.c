...
static void LED_Delay(void)
{
	uint8_t i;
	//while (cnt--)
		for(i=0; i<10; i++);//ouhs 20151109 for(i=0; i<10; i++);
}
...
#if 0
		//LED_G |= (1<<(DISP_WIDTH-pos_shift-1));	//使用ULN2003A
...
void TIM6_DAC_IRQHandler(void)
{
	...
	if (TIM_GetITStatus(TIM6, TIM_IT_Update) != RESET)
	{
        ...
        CursorTime++;
        if (CursorTime>=500
           && BIT(ApplVar.ScreenStat,(CURSOREN | CRITICAL))==CURSOREN)
        {
           mDispCursor();
           CursorTime = 0;
        }
        if (BIT(ApplVar.ScreenStat,CRITICAL))
            CursorTime = 0;
