...
uint16_t NAND_Write(uint32_t Memory_Offset, uint32_t *Writebuff, uint16_t Transfer_Length)
{
  ...
  //ouhs wAddress    = NAND_GetAddress(Memory_Offset / 512);
...
uint16_t NAND_Read(uint32_t Memory_Offset, uint32_t *Readbuff, uint16_t Transfer_Length)
{
  ...
  //ouhs phAddress = NAND_GetAddress(Memory_Offset / 512);
  ...
  else
  {
    ...
    //ouhs FSMC_NAND_ReadSmallPage ( (uint8_t *)Readbuff , phAddress, Transfer_Length / 512);
	FSMC_NAND_ReadSmallPage ( (uint8_t *)Readbuff , phAddress, PAGE_TO_WRITE);
  }
  return NAND_OK;
}
...
{
  SPARE_AREA t;
  uint8_t Buffer[NAND_SPARE_AREA_SIZE];//ouhs uint8_t Buffer[16];
...
uint8_t *ReadSpareAreas (uint32_t address,uint8_t *Buffer)
{
  NAND_ADDRESS address_s;
  address_s = NAND_ConvertPhyAddress(address);
  FSMC_NAND_ReadSpareArea(Buffer , address_s, 1) ;
...
uint8_t *WriteSpareAreas (uint32_t address,uint8_t *Buffer)
{
  NAND_ADDRESS address_s;
  
  address_s = NAND_ConvertPhyAddress(address);
  FSMC_NAND_WriteSpareArea(Buffer , address_s, 1) ;
...
static uint16_t NAND_Copy (NAND_ADDRESS Address_Src, NAND_ADDRESS Address_Dest, uint16_t PageToCopy)
{
  uint8_t Copybuff[NAND_PAGE_SIZE];//ouhs uint8_t Copybuff[512];
