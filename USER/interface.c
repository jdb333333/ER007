...
#define DST_STAR_MONTH	3		//夏时制开始月份(随后一个周日)
#define DST_END_MONTH		10	//夏时制结束始月份(随后一个周日)

#define DST_STAR_HOUR		3		//夏时制开始时间(DST_STAR_MONTH月最后一个周日)
#define DST_END_HOUR		4		//夏时制结束时间(DST_END_MONTH月最后一个周日)
...
BYTE DaylightSavingTimeSet(void)
{
	...
	if(SummerTimeEnabled())
	{
		...
		if((GetSystemTimer()-delayms)>=1000)	//1s
		{
				...
				march_day = (31-(5*years/4+5)%7);	// 偏差一天？？？march_day = (31-(5*years/4+4)%7);
				octo_day = (31-(5*years/4+2)%7);	//octo_day = (31-(5*years/4+1)%7);
				seconds = rtc_tmp.sec + rtc_tmp.min*60 + rtc_tmp.hour*60*60;

				if(RTC_GetStoreOperation() == RTC_StoreOperation_Set) //已启用夏令时
				{
					if((rtc_tmp.mon>DST_END_MONTH) ||
						((rtc_tmp.mon==DST_END_MONTH)&&(rtc_tmp.mday>octo_day)) ||
						((rtc_tmp.mon==DST_END_MONTH)&&(rtc_tmp.mday==octo_day)&&(seconds>=DST_END_HOUR*60*60))) 	//10月最后一个星期日AM4:00后取消夏令时
								{
				...
				else //没有启用夏令时
				{
					...
					((rtc_tmp.mon==DST_END_MONTH)&&(rtc_tmp.mday==octo_day)&&(seconds<(DST_END_HOUR-1)*60*60)) )	//3月最后一个星期日AM3:00后启用夏令时
								{
...
//
/*******************************************************************************
 * 清除外部RAM(掉电保护RAM)应用数据存储区,同时在EXTRAMN中标示SRAM块数(256K/块),
...
void RTC_AlarmSet(void)
{
  ...
  RTC_AlarmStructure.RTC_AlarmTime.RTC_H12     = RTC_H12_AM;  //RTC设置为RTC_HourFormat_24， AM和PM的设置是否自动无效？
  ...
  RTC_AlarmStructure.RTC_AlarmTime.RTC_Seconds = rtc.sec;
	...
	else if((rtc.mday==29)&&(rtc.mon==2))	//2月29日
	{
		RTC_AlarmStructure.RTC_AlarmDateWeekDay = 1;
	}
	else
...
/*********************************************************************************************************
** 仅用于开机时查询一次闹钟状态，即防止在关机状态下错过定时执行的任务
**
*********************************************************************************************************/
...
#if (1)//defined(CASE_ECR100F) || defined(CASE_ECR100S)	//ECR99和MCR30需要增加模拟U盘功能，PC访问NAND
	USB_Start();
#endif

#if defined(CASE_ETHERNET)
//ccr移到InitApplication之后	Ethernet_Start();
#endif
	...
	//读取一次RTC电池电压，并关闭RTC电压检测的ADC 因开启该ADC检测会导致RTC电池电流增大到26uA
	//开机只记录一次RTC电池电压  以后再调用pwrGetStatus获取的都是第一次的值
    ...
    RNG_Config();//ccr2017-10-20初始化随机数发生器

#if defined(NAND_DISK)	//ouhs20180327 只在有NAND Flash的机型上使用,用于NAND FTL
		my_mem_init(SRAMIN);
#else
		sf_Init();	//在没有NAND的机型上，才考虑使用SPI Flash
#endif
}

void Beep(uint8_t blen)
{
#if 1//jdb2019-03-08 去掉蜂鸣器
#if !defined(FOR_DEBUG)
    BEEP_ON();
    if (blen == 1)
        beep_len = 10;
...
void TIM6_Init(void)
{
    ...
    PrescalerValue = (uint16_t) ((SystemCoreClock / 2) / 1000000) - 1;
	/* Time base configuration */
    TIM_TimeBaseStructure.TIM_Period = 99999;//100ms    = 999; //1ms中断一次
