...
void NAND_DispBadBlockInfo(void)
{
	...
	xprintf("NAND Flash ID = 0x%04X, Type = ", id);
	if (id == HY27UF081G2A)
	{
		xprintf("HY27UF081G2A\r\n  1024 Blocks, 64 pages per block, 2048 + 64 bytes per page\r\n");
	}
	...
	xprintf("Block Info : 0 is OK, * is Bad\r\n");
	n = 0;	/* 坏块统计 */
	for (i = 0; i < NAND_BLOCK_COUNT; i++)
	{
		if (NAND_IsBadBlock(i))
...
void NAND_DispPhyPageData(uint32_t _uiPhyPageNo)
{
	...
	{
		xprintf("FSMC_NAND_ReadPage Failed() \r\n");
		return;
	}

	xprintf("Block = %d, Page = %d\r\n", ulBlockNo, usOffsetPageNo);
	...
	for (n = 0; n < 4; n++)
	{
		for (i = 0; i < 512; i++)
		{
			xprintf(" %02X", s_ucTempBuf[i + n * 512]);
	...
	for (i = 0; i < 64; i++)
	{
		xprintf(" %02X", s_ucTempBuf[i + 2048]);
...
void NAND_DispLogicPageData(uint32_t _uiLogicPageNo)
{
	...
	xprintf("LogicBlock = %d, PhyBlock = %d\r\n", _uiLogicPageNo, usPBN);
