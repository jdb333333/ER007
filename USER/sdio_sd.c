...
SD_CardInfo SDCardInfo;//jdb2018-09-03
...
extern SD_CardInfo SDCardInfo; //ccr2018-04-11
...
SD_Error SD_Init(void)
{
  ...
  if (SD_LastError == SD_OK)
  {
    SD_LastError = SD_EnableWideBusOperation(SDIO_BusWide_4b); //ouhs 20180403 1b --> 4b
  }
...
#if defined (SD_DMA_MODE) //ouhs20180504 从函数内部后面挪到前面实测提高SD卡访问速度
...
#if defined (SD_DMA_MODE)//ouhs20180504 从函数内部后面挪到前面实测提高SD卡访问速度	
  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_FLAG_TXUNDERR | SDIO_IT_STBITERR, ENABLE); //ouhs 20180423 修正ST官方库错误,解决偶尔卡机现象 SDIO_IT_RXOVERR-->SDIO_FLAG_TXUNDERR
...
#if defined (SD_DMA_MODE)	//ouhs20180504 从函数内部后面挪到前面实测提高SD卡访问速度
  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_TXUNDERR | SDIO_IT_STBITERR, ENABLE);	//ouhs 20180423
