...

{
	...
	if ( PortNum == LOGIC_COM2 )
	{
		...
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//ouhs 20170929 2; //提高优先级解决115200接收丢数据问题
...
//        UARTSend(LOGIC_COM1,XON);//ccr2014-08-27
	...
	else if ( PortNum == LOGIC_COM1 )
	{
		...
		NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//ouhs 2;  
...
//        UARTSend(LOGIC_COM2,XON);//ccr2014-08-27
	...
	else if ( PortNum == LOGIC_COM3 )
	{
		...
		/* USART2 GPIO config */
		GPIO_PinAFConfig(GPIOC, GPIO_PinSource6, GPIO_AF_USART6);  // USART6 Tx (PC.06)
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;
		GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;
		GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
		GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
		...
		GPIO_PinAFConfig(GPIOC, GPIO_PinSource7, GPIO_AF_USART6);	// USART6 Rx (PC.07)
		GPIO_InitStructure.GPIO_Pin = GPIO_Pin_7;
		GPIO_Init(GPIOC, &GPIO_InitStructure);
