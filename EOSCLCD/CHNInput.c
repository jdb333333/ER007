		...
		switch (GetFirmkeyID(keyno))
		{
			...
			case ID_xINPUT:
				...
				switch (ASCIIMode)
				{
					...
					case LOASCII:  DispCharXY('a',0,STATELINE);break;
					case UPASCII:  DispCharXY('A',0,STATELINE);break;
#ifndef CASE_MCR30 //jdb2018-09-05
					case SPASCII:  DispCharXY('*',0,STATELINE);break;
#endif
#if EN_EXASCII //ccr2014-03-26>>>>>>
					case EXASCII:
...
CONST char *T9KeysDef[24]={//jdb2019-03-05 九宫格
	".,|+-/_:\0",         //POINT
	"~",                //ZERO2
	"\x80%#""!'0\0",      //NUM0
	" ?()*@[]1\0",        //NUM1
...
BYTE LineEditor(union KEY ch)
{
	...
	int i, j;//jdb2019-03-05 九宫格
	static BYTE  CharOld=0;//jdb2019-03-05 上一次按键显示字符
	static ULONG keyDelay=0;//jdb2019-03-05 两次按键之间的间隔
	char saveEntryBuffer[80] = {0};
	...
	{//input is a character (HANZI or ASCII)
		...
		if((ch.C.Low>='a'|| ch.C.Low>='A') && (ch.C.Low<='l' || ch.C.Low<='L') && Appl_ProgLine > 0){//jdb2019-03-05 九宫格
			
			if(ASCIIMode == LOASCII){
				i = ch.C.Low - 'a';
				i += 12;
			}else{
				i = ch.C.Low - 'A';
			...
			j = 0;
			if(GetSystemTimer()<keyDelay){
				for(j = 0; j < strlen(T9KeysDef[i]); j++){
					if(CharOld == T9KeysDef[i][j]){
						break;
					}
				}
				j++;

				if(j <= strlen(T9KeysDef[i])){//找到字符
					memcpy(saveEntryBuffer, Editor.EditBuff, Editor.BuffSize);
					memcpy(Editor.EditBuff+1,saveEntryBuffer,Editor.BuffSize - 1);
					newX-=1;
					Editor.EditCount-=1;
				}

				if(T9KeysDef[i][j] == 0 || j >= strlen(T9KeysDef[i])){
					j = 0;
				}
			}

			Editor.EditBuff[oldX]=T9KeysDef[i][j];

			CharOld = Editor.EditBuff[oldX];
			keyDelay = GetSystemTimer()+SECONDS(1);
		}else{
			Editor.EditBuff[oldX]=ch.C.Low;
		}
			...
			//			if (Editor.EditCount<=Appl_MaxEntry)
			{
				...
				else
				{
					...
					if (ApplVar.CentralLock==RG || ApplVar.CentralLock==MG)
					{
						...
						{
#ifdef PUTS_2LINE//jdb2018-09-03
							cClearScreen();
							Puts1(SysBuf+Editor.DispW-DISLEN);
#else
