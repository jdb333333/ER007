			...
			if(TESTBIT(at_state, BIT0)){//发送AT命令成功，等待初始化完成
				...
				if(GPRS_AT() == false){//判断GPRS模块是否正常
					...
					if(GPRS_AT() == false){
						...
						if(!TESTBIT(at_state, SND_AT)){
							GPRS_Delay = GetSystemTimer()+SECONDS(15);
						}
...
BYTE GPRS_AT()
{
	...
	for (i=0;i<3;i++)
	{
		...
		memset(readGPRS, 0, sizeof(readGPRS));
		j=GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(2));
