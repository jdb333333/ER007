...
#define MyFlags(f)      BIT(ApplVar.MyFlags,f)//jdb2018-09-03
...
#define ID_DELETE   ID_RJFEED   //在编辑输入时,用作删除输入
	...
	#define  SECONDLAYERFR    30
    #define fCLEAR	24		//    35 is the scan code.
    #define fRJFEED	01			//    Set date
    #define fOCPRINTER	0xff			//    open or close printer ,lyq added 20040227
    #define fLOCK	0x00			//    Mode lock
    #define fREPORT	0xff			//  ReportList Trigger
    #define fPRGNUM	0xff         	//  Programming number
    #define fPRGUP	05         	//  Programming 向上选择项目
    #define fSYSREP	0xff         //  system report trigger
    #define fHARDTEST	0xff         //  Machine test key
    #define fDUMP	0xff         //  Dump Program key
    #define fDOWN	11         //  Dump Program key Left or Down
	//ccr2017-12-15 直接使用SHIFT1	  #define fSHIFTDEPT	0xff		 //  Alfa Shift 1 (xchange ASCII<->QuWei)
    #define fDATE	(17+SECONDLAYERFR)	//ccr2018-05-07*****	//    feed paper anytime    */
    #define fPRGTYPE	63          //  programming Type key
    #define fPRGENTER	29          //  programming Enter
    #define fSHIFTKEY	03
    #define fSHIFT3	0xff        //  Alfa Shift 3
    #define fCANCEL     62			//  虚拟退出  cancel,EXIT
    #define fBACKSPACE	fRJFEED		//输入字符数据时,用来回删字符的按键
...
#define DHCPFlag        BIT0    //启用DHCP方式
#define IPByDHCPFlag    BIT1    //启用DHCP方式时,得到了IP地址,不启用DHCP时,此标志自动设置
#define MINISTRYFlag    BIT2
#define DNSPROGRESSFlag BIT3    //已经提交DNS域名解析
#define NETENABLEFlag   BIT4    //ccr2018-06-14
#define SYNSERVERTIMEflag   BIT5    //ccr2018-08-14 当上传S文件后,是否与服务器同步时间
#define PRINTNETSET     BIT7 //是否打印菜单
...
struct TNETWORK
{
    ...
    char    UserName[16];
    char    Password[16];
    BYTE    ECR_ID[10];  //ccr2017-09-14PERSONAL CODE NUMBER 9位数字
...
struct APPLICATION
{
    ...
    struct ICBLOCKTABLE ICBlock;  //ccr chipcard 2004-07-01
		struct TNETWORK NetWork;//jdb2018-09-02
