...
#define GPRS_CLOSE_DEBUG //jdb2019-03-07关闭GPRS功能
...
#if defined(GPRS_SIM800)//jdb2019-02-27 GPRS_SIM800
 ...
 */
#if defined(GPRS_SIM800)//jdb2019-02-27 GPRS_SIM800
#define GPRS_INI_OK BIT0
#define GPRS_SIM_OK BIT1
#define GPRS_IFS_OK BIT2
#define GPRS_CSE_OK BIT3
#define GPRS_RES_OK BIT4

static unsigned char gprs_state = 0x00;
static unsigned long GPRS_Delay = 0;
static unsigned long GPRS_CSE_Delay = 0;

#define SND_AT BIT0
#define SND_CPIN BIT1
#define SND_CSTT BIT2
#define SND_CIISR BIT3
#define SND_CIFSR BIT4
#define SND_CIPSTART BIT5

static unsigned char at_state = 0;
//BIT0 1 GPRS上电后，发送AT命令成功 0 没有发送成功
//BIT1 1 发送了CPIN命令，检测SIM卡状态 0 没有发送命令
//BIT2 1 发送了CSTT命令，设置APN 0 没有发送命令
//BIT3 1 发送了CIISR命令 0 没有发送命令
//BIT4 1 发送了CIFSR命令 0 没有发送命令
void GPRSWaitForReady(BYTE init)
{
	static int rdyPOS=-1;
	short j;
	static BYTE gprsOK = false;
	char readGPRS[64];
	int retv;
	static short rcvcnt = 0;

#if defined(GPRS_CLOSE_DEBUG)
	return;
#endif

	if (gprsOK == false)
	{
		if(!TESTBIT(gprs_state, GPRS_INI_OK)){
			if(TESTBIT(at_state, BIT0)){//发送AT命令成功，等待初始化完成
				if(rcvcnt >= 30){
					return;
				}
				
				if(GetSystemTimer()>GPRS_Delay){
					memset(readGPRS, 0, sizeof(readGPRS));
					j=GPRSReadAString(readGPRS,sizeof(readGPRS),100);
					if(j > 0){
						GPRSDataDelRD(readGPRS, strlen(readGPRS));
						if(strstr(readGPRS,"SMS Ready")){
							DispSt1("SMS Ready         ");
							SETBIT(gprs_state, GPRS_INI_OK);
							return;
						}
					}
					rcvcnt++;
					GPRS_Delay = GetSystemTimer()+SECONDS(1);
				}
				...
				if(retv < 0){//判断GPRS模块是否正常
					if(!TESTBIT(gprs_state, GPRS_RES_OK)){
						RESETBIT(at_state, SND_AT);
						DispSt1(GPRSMess[G_WAITFOR].str);

						SIM800_PowerOn();
						rcvcnt = 0;
						GPRS_Delay = GetSystemTimer()+SECONDS(2);
						SETBIT(gprs_state, GPRS_RES_OK);
					}else{//已经重启过GPRS模块，发送AT命令还是出错，直接返回
						return;
					}
				}else if(retv > 0){
					SETBIT(at_state, SND_AT);
					return;
				}
			}
		}else{
			if (!TESTBIT(gprs_state, GPRS_SIM_OK))//SIM没有就绪
			{
				// (!GPRSSIM_Exist())
				retv = GPRS_ATCPIN();
				if (retv < 0)
				...
				if(!TESTBIT(gprs_state, GPRS_IFS_OK)){
					if(GPRS_APN_CSD()){
					}else{
						DispSt1("                         ");
						mSetInvAttr();
						DispSt1(GPRSMess[G_GPRSNOTOK].str);
						mClearInvAttr();
					}
				}else{
					gprsOK = true;
 ...
 */
#if defined(GPRS_SIM800)//jdb2019-02-27 GPRS_SIM800
void GPRSDealy(ULONG sWaitFor)
{
	SetTimerDelay0(sWaitFor);
	while(GetTimerDelay0()>0);
}

short GPRSDataDelRD(BYTE *str, short size)
{
	short slen, dlen;
	BYTE data;

	for(slen = 0, dlen = 0; slen < size; slen++){
		data = str[slen];
		if(data == '\r' || data == '\n'){
		}else{
			str[dlen++] = data;
...
{
	char readGPRS[32];
	static BYTE sndAt = false, readcnt = 0;
	...
	if(!sndAt){
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_CMD,strlen(AT_CMD));
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAt = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
		j=GPRSReadAString(readGPRS,sizeof(readGPRS),100);
		if(strstr(readGPRS,"OK")){
			return 1;
		}else{
			EmptyComm(GPRSPORT);//清接收缓存
			GPRSSendAString(AT_CMD,strlen(AT_CMD));
...
{
	char readGPRS[32];
	static BYTE sndAtCpin = false, readcnt = 0;
	...
	if(!sndAtCpin){
		...
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_CPIN,strlen(AT_CPIN));
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAtCpin = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
		j=GPRSReadAString(readGPRS,sizeof(readGPRS),100);
		if(strstr(readGPRS,"+CPIN: READY")){
			SETBIT(gprs_state, GPRS_SIM_OK);//SIM卡OK
			return 1;
		}else{
			GPRS_Delay = GetSystemTimer()+SECONDS(1);
			readcnt++;
...
CONST char AT_SIM[]="AT+CPIN?\r\n";
BYTE GPRSSIM_Exist()
{
	char readGPRS[32];
	BYTE simOK=true;
	static BYTE simcheck = false;//是否已经检测过SIM卡

	short i,j;

	if(!simcheck){
		if(!TESTBIT(at_state, SND_CPIN)){//还没有发送CPIN命令
			GPRSSend_ATE0();//关闭回显

			EmptyComm(GPRSPORT);//清接收缓存
			GPRSSendAString(AT_SIM,strlen(AT_SIM));
			SETBIT(at_state, SND_CPIN);
			GPRS_Delay = GetSystemTimer()+SECONDS(2);//等待2S后再获取AT返回值
		}else{
			if(GetSystemTimer()>GPRS_Delay){
				memset(readGPRS, 0, sizeof(readGPRS));
				j=GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(10));
				//GPRSDataDelRD(readGPRS, j);
				if(strstr(readGPRS,"+CPIN: READY")){
					SETBIT(gprs_state, GPRS_SIM_OK);//SIM卡OK
				}else{
					simOK = false;
				}
				simcheck = true;//已经检测过SIM卡，不用再次检测
			}
		}
	}
...
CONST char AT_APN[]="AT+CSTT=\"CMNET\"\r\n";
CONST char AT_CSD[]="AT+CIICR\r\n";
CONST char AT_CIFSR[]="AT+CIFSR\r\n";

#if 1
CONST char AT_CSTT[]="AT+CSTT=\"CMNET\"\r\n";

int GPRS_ATCSTT()
{
	char readGPRS[32];
	static BYTE sndAtCstt = false, readcnt = 0;

	short j;
	static BYTE apnOk = false;
	...
	if(!sndAtCstt){
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_CSTT,strlen(AT_CSTT));
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAtCstt = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
...
CONST char AT_CIISR[]="AT+CIICR\r\n";

int GPRS_ATCIISR()
{
	char readGPRS[32];
	static BYTE sndAtCiisr = false, readcnt = 0;

	short j;
	static BYTE csdOk = false;
	...
	if(!sndAtCiisr){
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_CIISR,strlen(AT_CIISR));
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAtCiisr = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
...
{
	char readGPRS[32];
	static BYTE sndAtCifsr = false, readcnt = 0;

	short j;
	static BYTE cifsrOk = false;
	...
	if(!sndAtCifsr){
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_CIFSR,strlen(AT_CIFSR));
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAtCifsr = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
...
{
	char readGPRS[32];
	static BYTE apnOK=false, csdOK = false, ifsOK = false;
	static BYTE checkapn = false;//是否已经检测过APN
	static BYTE checkcsd = false;//是否已经检测过SCD
	static BYTE checkifs = false;//是否已经检测过IFS
...
#if 0
	if(!TESTBIT(at_state, SND_CSTT)){
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(AT_APN,strlen(AT_APN));
		SETBIT(at_state, SND_CSTT);
		GPRS_Delay = GetSystemTimer()+SECONDS(2);//等待2S后再获取AT返回值
		apnOK = false;
		checkapn = false;
	}else{
		if(!checkapn){
			if(GetSystemTimer()>GPRS_Delay){
				memset(readGPRS, 0, sizeof(readGPRS));
				j=GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(10));
				if(strstr(readGPRS,"ERROR")){
					DispSt1("APN IS ERROR");
			...
			if(apnOK){
				if(!TESTBIT(at_state, SND_CIISR)){
					EmptyComm(GPRSPORT);//清接收缓存
					GPRSSendAString(AT_CSD,strlen(AT_CSD));
					SETBIT(at_state, SND_CIISR);
					GPRS_Delay = GetSystemTimer()+SECONDS(5);//等待5S后再获取AT返回值
					csdOK = false;
					checkcsd = false;
				}else{
					if(!checkcsd){
						...
						if(csdOK){
							if(!TESTBIT(at_state, SND_CIFSR)){
								EmptyComm(GPRSPORT);//清接收缓存
								GPRSSendAString(AT_CIFSR,strlen(AT_CIFSR));
								SETBIT(at_state, SND_CIFSR);
								GPRS_Delay = GetSystemTimer()+SECONDS(5);//等待5S后再获取AT返回值
								ifsOK = false;
								checkifs = false;
							}else{
...
CONST char AT_IPSTATUS[]="AT+CIPSTATUS\r\n";
CONST char AT_TCPSTART[]="AT+CIPSTART=\"TCP\",";
CONST char AT_TCPCLOSE[]="AT+CIPCLOSE\r\n";
CONST char ATTEST_TCPSTART[]="AT+CIPSTART=\"TCP\",\"116.196.67.124\",\"55391\"\r\n";
CONST char AT_TCPSENDTEST[]="hello server tcp";

BYTE GPRS_TCP_STATUS()
{
	char readGPRS[32];
	short i, j;

	EmptyComm(GPRSPORT);//清接收缓存
	GPRSSendAString(AT_IPSTATUS,strlen(AT_IPSTATUS));

	memset(readGPRS, 0, sizeof(readGPRS));
	j=GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(1));

	if(strstr(readGPRS, "CONNECT OK")){
		SETBIT(gprs_state, GPRS_CSE_OK);
		return true;
	}else{
		GPRS_TCP_CLOSE();
		RESETBIT(at_state, SND_CIPSTART);
		RESETBIT(gprs_state, GPRS_CSE_OK);
		return false;
	}
}
...
BYTE GPRS_TCP_CLOSE()
{
	GPRSSendAString(AT_TCPCLOSE,strlen(AT_TCPCLOSE));
	RESETBIT(gprs_state, GPRS_CSE_OK);
	DispSt1("close tcp connect");

	return true;
}

void GPRS_RESET_DELAY()
{
	GPRS_CSE_Delay = GetSystemTimer()+SECONDS(60);//1分钟内保持连接服务器
}

CONST char ATIPOPEN[]="AT+CIPSTART=\"TCP\",\"";
CONST char AT_CIPSTART[]="AT+CIPSTART=\"TCP\",\"116.196.67.124\",\"55391\"\r\n";
int GPRS_ATCIPSTART()
{
	char readGPRS[32];
	static BYTE sndAtCipstart = false, readcnt = 0;

	short j, len;
	static BYTE connectOk = false;

	if(!TESTBIT(at_state, SND_CIPSTART)){
		connectOk = false;
		readcnt = 0;
		sndAtCipstart = false;
	}
	...
	if(!sndAtCipstart){
		strcpy(PCBuffer,ATIPOPEN);
		len=strlen(ATIPOPEN);//get the TCP/IP command
		...
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(PCBuffer,strlen(PCBuffer));
		SETBIT(at_state, SND_CIPSTART);//jdb2019-03-12已经发送CIPSTART命令
		GPRS_Delay = GetSystemTimer()+SECONDS(1);
		sndAtCipstart = true;
	}else if(GetSystemTimer() > GPRS_Delay){
		memset(readGPRS, 0, sizeof(readGPRS));
...
BYTE GPRS_TCP_CONNECT()
{
	char readGPRS[32];
	char AT_TCPCONNET[64] = {0};
	short i, j;
	BYTE connectOK = false;
	int retv;

	if(!TESTBIT(gprs_state, GPRS_CSE_OK)){
		retv = GPRS_ATCIFSR();

		if(retv > 0){
			retv = GPRS_ATCIPSTART();
			if(retv > 0){
				SETBIT(gprs_state, GPRS_CSE_OK);
				DispSt1("CONNECT OK   ");
				return true;
	...
	if(!TESTBIT(gprs_state, GPRS_CSE_OK)){
		...
		EmptyComm(GPRSPORT);//清接收缓存
		GPRSSendAString(ATTEST_TCPSTART,strlen(ATTEST_TCPSTART));
		...
		for(i = 0; i < 2; i++){
			...
			if(strstr(readGPRS, "CONNECT OK")){
				...
				if(!GPRS_TCP_STATUS()){//再次确认是否连接出错
					DispSt1("CONNECT ERROR");
					RESETBIT(gprs_state, GPRS_CSE_OK);
					GPRSDealy(SECONDS(2));
		...
		if(GetSystemTimer() > GPRS_CSE_Delay){//TCP空闲状态超时
			GPRS_TCP_CLOSE();
		}else{
			return true;
		}
	...
	if(connectOK == true){
		//GPRSSendECR_LOG();
		GPRSSendECRData(AT_TCPSENDTEST, strlen(AT_TCPSENDTEST));
	}


	while(1){
		memset(readGPRS, 0, sizeof(readGPRS));
		j = GPRSRead_IPDATA(readGPRS, sizeof(readGPRS),SECONDS(10));
		//j = GPRSReadAString(readGPRS, sizeof(readGPRS),SECONDS(10));
		if(j > 0){
			DispSt1(readGPRS);
		}
...
#if defined(GPRS_SIM800)//jdb2019-02-27 GPRS_SIM800
CONST char ATIPSEND[]="AT+CIPSEND\r\n";
BYTE GPRS_WaitSendOk(BYTE *str,short size,ULONG sWaitFor)
{
	short i,j;
	BYTE sEnd=0;
	...
	{//有数据时才发送
		...
		if(strstr(readGPRS, ">")){
			for (i=0;i<len;i++)
				...
				GPRSSendByte(buffer[i]);
			GPRSSendByte(0x1a);//命令结束

			memset(readGPRS,0,sizeof(readGPRS));
#if 1
			if(GPRS_WaitSendOk(readGPRS,sizeof(readGPRS),SECONDS(5))){
				DispSt1("SEND DATA OK");
				return true;
			}else{
				DispSt1("SEND DATA FAILED");
				//GPRSDealy(SECONDS(2));
			}
#else
			j = GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(5));
			if(strstr(readGPRS, "SEND OK")){
				DispSt1("SEND DATA OK");
				return true;
			}else{
				DispSt1("SEND DATA FAILED");
			...
			DispSt1("SEND DATA ERROR");
			//GPRSDealy(SECONDS(2));
		}
	}

	//GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(5));
...
#if 1
	j = GPRSReadAString(strOK,sizeof(strOK),SECONDS(10));
	if(j >= 7 && strstr(strOK, REQUEST))
		return true;
...
#if 1
	j = GPRSReadAString(strOK,sizeof(strOK),SECONDS(10));
	if(j >= 7 && strstr(strOK, DATASUCCESS))
		return true;
...
//#define GPRS_FORDEBUG
extern  BYTE PrBuffer2[];

#if defined(GPRS_FORDEBUG)//jdb2019-03-04调试用
void GPRSSendECR_LOG()
{
	...
	if (GPRSStartGprs())
	{
		...
		size = 0;
		logSize = 0;
		memcpy(PCBuffer+logSize,(BYTE *)PrBuffer2,
				strlen(PrBuffer2));
		logSize+=strlen(PrBuffer2);
		...
		{//有要发送的数据,将数据发送出去
			...
			sprintf(INFO_ALL + strlen(INFO_ALL), "%d-%d", 1, 1);
			...
			{
				DispSt1("LOG SEND ERR          ");
				return;
			}

			if (!GPRSWaitForRequest(SECONDS(10)))
			{
				DispSt1("LOG RCV ERR1    ");
				return;//没有收到 "REQUEST"
			}
...
void GPRSSendECR_LOG()
{
	short i,size;
	ULONG sendCount=0;//发送计数器
	static ULONG logReadCopyNext=0;//下一条即将要读取的流水数据
	static ULONG logReadCopyLast=0;//当前读取的流水数据
	BYTE OK_Send;
	ULONG logSize;//用来指定打包发送的日志数据包大小
	int  sendFlag;//发送计数器
	struct TimeDate sDate;
			...
			for (i=0;i<3;i++)
			{
				if ((j=GPRSReadAString(readGPRS,sizeof(readGPRS),SECONDS(10)))>=0)
				{
					...
					if (GPRSHand(readGPRS,"+CSQ:"))
					{
						...
						if (s>31) s=0;
						//mDrawGPRSSignal(s,SIGNAL_X,SCREENLN-1);//jdb2018-09-03
					}
				}
				else
					break;
