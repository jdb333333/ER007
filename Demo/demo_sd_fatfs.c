...
void DemoFatFS_SD(void)
{
	...
	{
		xprintf("ERROR:SD is not present!");
		return;
	}
	else
	{
		...
		if (SD_Status == SD_OK)
		{
			xprintf(" CardBlockSize is ..%d\r\n", SDCardInfo.CardBlockSize);
			xprintf(" CardCapacity is ..%d M\r\n", SDCardInfo.CardCapacity/1024 /1024);	//uint64_t 编译有问题？
			xprintf(" CardType is ..%d\r\n", SDCardInfo.CardType );
			xprintf(" RCA is ..%d\r\n", SDCardInfo.RCA);
			xprintf(" ManufacturerID is ..%d\r\n", SDCardInfo.SD_cid.ManufacturerID );
		}
		else
		{
			xprintf("ERROR:SD Init failed!\r\n");
		}
	}	
...
static void CreateNewFile(void)
{
	...
	/* 打开文件 */
	result = f_open(&file, "1:/test.txt", FA_CREATE_ALWAYS | FA_WRITE);
...
static void ReadFileData(void)
{
	...
	/* 打开文件 */
	result = f_open(&file, "1:/test.txt", FA_OPEN_EXISTING | FA_READ);
	if (result !=  FR_OK)
	{
...
static void CreateNewDirFileTest(void)
{
	const uint8_t WriteText[] = {"1.STM32 Demo application\r\n2.STM32 Demo application\r\n3.STM32 Demo application\r\n4.STM32 Demo application\r\n20180420"};
...
static void ViewNandCapacity(void)
{
	...
	//读取SD卡容量及剩余容量
	DisplayTime(0);
	if(f_getfree("1:/", &free_clust, &pfs)==FR_OK)
	{
