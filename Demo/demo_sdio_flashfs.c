...
static void ViewNandCapacity(void)
{
	...
	/* 
	   ...
	   1. finit("M0:") 会调用NAND_Init，见FlahFS对nandflash的接口文件FS_NAND_FlashPrg.c
	   2. funinit("M0:") 会调用UnInit，见FlahFS对nandflash的接口文件FS_NAND_FlashPrg.c
	*/
	NAND_Init();
	xprintf("正在进行低级格式化中....\r\n");
	NAND_Format();
...
static void CreateNewFile(void)
{
	...
	if (fout != NULL) 
	{
		xprintf("打开文件M0:\\test\\test1.txt成功，如果没有子文件夹和txt文件会自动创建\r\n");
		/* 写数据 */
	...
	{
		xprintf("打开文件M0:\\test\\test.txt失败\r\n");
	}
	
	/**********************************************************************************************************/
	/* 打开文件夹test中的文件test2.txt，如果没有子文件夹和txt文件会自动创建*/
	fout = fopen ("M0:\\test\\test2.txt", "w"); 
	if (fout != NULL) 
	{
		xprintf("打开文件M0:\\test\\test2.txt成功，如果没有子文件夹和txt文件会自动创建\r\n");
	...
	fout = fopen ("M0:\\test\\test3.txt", "w"); 
	if (fout != NULL) 
	{
		xprintf("打开文件M0:\\test\\test3.txt成功，如果没有子文件夹和txt文件会自动创建\r\n");
...
static void ReadFileData(void)
{
	...
	fin = fopen ("M0:\\test\\test1.txt", "r"); 
	if (fin != NULL) 
	{
		xprintf("<1>打开文件M0:\\test\\test1.txt成功\r\n");
	...
	{
		xprintf("打开文件M0:\\test\\test.txt失败, 可能文件不存在\r\n");
	}
	
	/**********************************************************************************************************/
	/* 打开文件夹test中的文件test2.txt */
	fin = fopen ("M0:\\test\\test2.txt", "r"); 
	if (fin != NULL) 
	{
	...
	/**********************************************************************************************************/
	/* 打开文件夹test中的文件test3.txt，如果没有子文件夹和txt文件会自动创建*/
	fin = fopen ("M0:\\test\\test3.txt", "r"); 
	if (fin != NULL) 
	{
		xprintf("\r\n<3>打开文件M0:\\test\\test3.txt成功\r\n");
...
static void SeekFileData(void)
{
	...
	if (fout != NULL) 
	{
		xprintf("打开文件M0:\\test.txt成功，如果没有txt文件会自动创建\r\n");
		/* 写数据 */
		bw = fwrite (WriteText, sizeof(uint8_t), sizeof(WriteText)/sizeof(uint8_t), fout);
		if(bw == sizeof(WriteText)/sizeof(uint8_t))
		{
	...
	{
		xprintf("打开文件M0:\\test.txt失败\r\n");
	}
	
	/***********************************************/
	fin = fopen ("M0:\\test.txt","r");
	if (fin != NULL)  
	{
		xprintf("\r\n打开文件M0:\\test.txt成功\r\n");
...
static void DeleteDirFile(void)
{
	...
	for (i = 0; i < 20; i++)
	{
		xprintf(FileName, "M0:\\Speed%02d.txt", i);		/* 每写1次，序号递增 */
	...
	for(i = 1; i < 4; i++)
	{
		xprintf(FileName, "M0:\\test\\test%01d.txt", i);   /* 每写1次，序号递增 */
		result = fdelete (FileName);
		if (result != NULL) 
		{
			xprintf("%s文件不存在（返回值：%d）\r\n", FileName, result);
...
static void WriteFileTest(void)
{
	...
	/* 打开文件 -----------------------------------------*/
	sprintf(TestFileName, "M0:\\Speed%02d.txt", s_ucTestSn++);		/* 每写1次，序号递增 */
...
static void WriteCSVFile(void)
{
	...
	*/
	fout = fopen ("M0:\\record.csv", "a"); 
	if (fout != NULL) 
	{
		xprintf("打开文件M0:\\record.csvt成功，如果没有此文件会自动创建\r\n");
...
static void ReadCSVFileData(void)
{
	...
	*****************************************************/
	/* 打开record.csv文件 */
	fin = fopen ("M0:\\record.csv", "r"); 
	if (fin != NULL) 
	{
		xprintf("打开文件M0:\\record.csv成功\r\n");
	...
	{
		xprintf("打开文件M0:\\record.csv失败，可能文件不存在，已经被删除\r\n");
	}
...
static void CreateNewDirFileTest(void)
{
	...
	while(Filenum++<20000)
	{
		sprintf(filename, "M0:\\dir%d-%02d\\dir2-%02d\\dir3-%02d\\dir4-%02d\\Test%02d.txt", (uint8_t)rng_data,Filenum,Filenum,Filenum,Filenum,Filenum);
		xprintf("Open file\r\n");
		xprintf(filename);
		DisplayTime(0);
...
void MountSD(void)
{
	...
	if (ioc_init (mc0) == 0) 
	{
		...
		xprintf("SD卡总容量 = %10s字节\r\nSD卡的总扇区数 = %d \r\n", buf, info.block_cnt);
	
		xprintf("SD卡读扇区大小 = %d字节\r\n", info.read_blen);
		xprintf("SD卡写扇区大小 = %d字节\r\n", info.write_blen);
	}
	else 
	{
		xprintf("初始化失败\r\n");
