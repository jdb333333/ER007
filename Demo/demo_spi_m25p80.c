...
void DemoSpiFlash_FM(void)
{
	...
	else
	{
		...
		xprintf("检测到串行Flash, ID = %08x, RES = %08x",ChipID,ChipRES);
...
static void sfErase(void)
{
	...
	for (i = 0; i < FiscalMemSize / FiscalMem_SPI_PAGESIZE; i++)
	{
		FiscalMem_ReadBuffer(buf, i * FiscalMem_SPI_PAGESIZE, FiscalMem_SPI_PAGESIZE);		/* 读数据 */
...
static void sfViewData(uint32_t _uiAddr)
{
	...
	FiscalMem_ReadBuffer(buf, _uiAddr,  1024);		/* 读数据 */
