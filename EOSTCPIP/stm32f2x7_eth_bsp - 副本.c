...
/**
将以太网MAC地址写入OTP Flash，有两个地址用于存放MAC：OTP_FLASH_ADDR(优先)和OTP_FLASH_ADDR+6(备用)
1.写MAC前先读取OTP_FLASH_ADDR是否为全0xff
2.若全为0xff，则将MAC写入OTP_FLASH_ADDR地址
  若不全为0xff，则将MAC与读取的内容比较，相同，则放弃写操作；不同，则将MAC写入OTP_FLASH_ADDR+6地址
  */
uint8_t OTPFlash_WriteMAC(uint8_t* pBuffer)
{
	...
	uint8_t i;
	uint32_t MACAddr;
	uint8_t rBuff[6],tmpBuff[6];
	
	//读取MAC_ADDR的数据	
	memcmp(rBuff, (__IO uint8_t*) OTP_FLASH_ADDR, 6);
	memset(tmpBuff, 0xff, 6);
	if(memcmp(rBuff, tmpBuff, 6)==0) //MAC_ADDR地址全为0xff, 空
	{
		MACAddr = OTP_FLASH_ADDR;	//写入MAC_ADDR
	}
	else if(memcmp(rBuff, pBuffer, 6)==0) //已存在相同的MAC地址
	{
		return SUCCESS;
	}
	else
	{
		MACAddr = (OTP_FLASH_ADDR+6);	//写入备用地址MAC_ADDR
	}
	
	FLASH_Unlock();
  for (i=0; i<6; i++)
  {
    FLASHStatus = FLASH_ProgramByte(MACAddr, *pBuffer);
	...
	//读出，验证写入是否正确
	MACAddr -= 6;	
	if(memcmp((__IO uint8_t*) MACAddr, (pBuffer-6), 6)==0)		
		return (SUCCESS);
	else
		return (ERROR);
}
...
优先读取MAC_ADDR1，若全为0xff，则读取MAC_ADDR
  */
uint8_t OTPFlash_ReadMAC(uint8_t* pBuffer)
{
	...
	//读取(OTP_FLASH_ADDR+6)的数据
	memcpy(rBuff, (__IO uint8_t*) (OTP_FLASH_ADDR+6), 6);
	memset(tmpBuff, 0xff, 6);
	
	if(memcmp(rBuff, tmpBuff, 6)==0) //(OTP_FLASH_ADDR+6)地址全为0xff,空
		MACAddr = OTP_FLASH_ADDR;	
	else
		MACAddr = (OTP_FLASH_ADDR+6);	
	
	memcpy(pBuffer, (__IO uint8_t*) MACAddr, 6);
