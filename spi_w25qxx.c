...
void sf_Init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_4;	  //SPI_BaudRatePrescaler_2 长缆较长时不稳定 SPI1位APB2总线下，速度最大可达30MHz  //
	...
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			...
			case W25Q16DV_ID:
				...
				g_tSF.TotalSize = 2 * 1024 * 1024;	/* 总容量 = 2M */
				g_tSF.PageSize = 256;//4 * 1024;			/* 页面大小 = 256 byte */
				break;			
			
			case W25Q64BV_ID:
				strcpy(g_tSF.ChipName, "W25Q64BV");
				g_tSF.TotalSize = 8 * 1024 * 1024;	/* 总容量 = 8M */
				g_tSF.PageSize = 256;//4 * 1024;			/* 页面大小 = 4K */
				break;
...
uint8_t bFlashMBlockErase(uint32_t ucBlock, uint32_t ucNumBlocks)
{
	...
	// 检查入口参数
	if ((ucBlock > SFLASH_SECTOR_MAXNUM)||((ucBlock+ucNumBlocks) > SFLASH_SECTOR_MAXNUM) || (ucNumBlocks==0))
  {
		return (ERROR);
  }
	
  // 若从0 sector擦除数量为SFLASH_SECTOR_MAXNUM,则执行芯片擦除
  if ((ucBlock == 0) && (ucNumBlocks == SFLASH_SECTOR_MAXNUM))
  {
