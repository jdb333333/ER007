...
#ifndef BARIMGHIGH
#define BARIMGHIGH   50     //条码图片的高度
                           //希腊需要打印27个字符的收据结账金额的条码:"EUT17000001000007400005632"
#if defined(CASE_GREECE)
#define BARCODESIZE  ((BARIMGHIGH+1)*27) //ccr2017-08-09 实际需要的字节数为1350字节
#else
#define BARCODESIZE 0  //ccr2017-08-09 实际需要的字节数为2160字节
#endif
#endif
...
#define PRN_RECEIPT_LINE     BIT6       //ccr2018-01-31 打印收据行
#define PRN_BOLD_MODE        BIT7    //ccr2018-07-30 打印粗体行
...
#define BEATS_PER_CYCLE 8 		//电机换相周期为8拍

#elif	defined(CASE_ECR99) || defined(CASE_MCR30)

#define RPH1     GPIO_Pin_6
#define RPH2     0				//RPH1和RPH2已在硬件电路上实现反相
#define RPH3     GPIO_Pin_7
#define RPH4     0 				//RPH3和RPH4已在硬件电路上实现反相
#define BEATS_PER_CYCLE 4 		//电机换相周期为4拍
...
#define GROUPS          6             //groups for hot 分组加热组数 //未使用 HJ
#if (BEATS_PER_CYCLE==8)
#define STEPS1LINE      4   //每行加热4次,即左右各2次
#define PHASEMAX        8	//换相周期为8拍
#define VMAXSPEED       60	//最快打印速度80mm/s
#else
#define STEPS1LINE      2   //每行加热两次,即左右各一次
#define PHASEMAX        4	//换相周期为4拍
#define VMAXSPEED       8	//最快打印速度70mm/s
...
#define PrnHotON() { }     //打开加热
#define PrnHotOFF() {GPIOB->BSRRH = HOTBIT123 | HOTBIT456;}    //关闭加热
...
#define PrnPowerON() {GPIOD->BSRRL = POWERBIT;} //打开电源
#define PrnPowerOFF() {GPIOD->BSRRH = POWERBIT;}    //关闭电源
...
#define POWERBIT    GPIO_Pin_3      // PD.3 打印机电源开关位
...
typedef volatile struct
{
    ...
    ULONG g_bHold            :1;     //ccr2018-03-01 数据加入队列后不立即打印
    ...
    ULONG g_bHotOver         :1;     // 加热时间大于步进时间
    ULONG g_bClean           :1;     //ccr2018-01-26清除所有打印任务
#if (CASE_FTP628MCL401) //HJ
    ULONG g_bHotLatch         :1;     //HJ只在锁存后加热,用来控制二次加热
#endif
    ULONG g_bDisconnect       :1;    //ccr2018-04-02 没有打印机
} PRNSTATUS;
...
extern int RSPrint(BYTE *buf, int count, BYTE prnCMD);//jdb2018-09-02
