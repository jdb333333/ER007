...
#if ENCUTTER
    else if (BIT(Prn_Mode, PRN_CUTF_Mode ))
    {//cut paper
        Prn_task16_CutPaper();
    }
...
#if ENCUTTER
        else  if (BIT(Prn_Mode, PRN_CUTF_Mode  | PRN_CUTRESET_Mode))
        {//cut paper
            Prn_TaskNow = -1;
...
#if ENCUTTER
    // 切纸工作模式下,不检测无纸等状态
    if (!BIT(Prn_Mode,PRN_CUTRESET_Mode | PRN_CUTF_Mode ))
#endif
    {
        Check_Print();
...
#if ENCUTTER
        if (BIT(Prn_Mode,PRN_CUTRESET_Mode | PRN_CUTF_Mode ) && Prn_TaskNow!=TASK19_RevFeed8Line)
        {
